...
///////////////////////////////////////////////////////////////////////
void render::createfbo(GLuint &fbo, GLuint &tex, int tid, int hei, int wid) {
	// GLuint fbo = 0;
...
void render::createbigtex() {
	// GLuint tex_screen;
	glGenTextures(1, &tex_saver);
	glActiveTexture(GL_TEXTURE0 + tid);
	...
	if (maxsz * width > maxwidth) {
		cout << "maxsz * width\t" << maxsz * width << endl;
		cout << "too big maxsz" << endl;
		exit(1);
	}

	maxwidth = width * maxsz;
...
void render::initializecuda() {
	...
	tid = 1;
	createfbo(fbo_acc, tex_acc, tid, height, width);
